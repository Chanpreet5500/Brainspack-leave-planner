...
const postForgotEmail = async(data) => {
  return await axiosInstance.post('http://localhost:5233/forgot-password', data);
}

const getLeaveDataById = async(id, userType) => {
  return await axiosInstance.get(`http://localhost:5233/leave-data/${id}/${userType}`)
}
...
export const GetLeaveDataById = (id, userType) => {
  return useQuery(['get-leave-data-id', id, userType], () => getLeaveDataById(id, userType), {retry: false})
}
